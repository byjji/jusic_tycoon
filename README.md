...
- **사운드**: 주행 굉음/바람/리프트 클랙 등 모든 효과음을 Web Audio API로 실시간 합성 (음원 파일 없음).
- **엔딩 분기**
  - 상승 마감 → 🚀 우주로 발사
  - 하락 마감 → 💥 땅에 곤두박질치며 파괴
  - 평탄한 마감 → 🛑 정거장에 안전 정지
- **종목/기간 선택**: 텍스트 검색 + 칩으로 종목 선택, 월봉(5년)/1년(주봉)/3개월(일봉) 기간 선택, 미니 차트 미리보기.
...
데이터는 [js/data.js](js/data.js)의 종목별 기준가(anchors) + 고정 시드 노이즈로 생성되어 항상 동일한 차트가 나옵니다.
실시간 시세가 아닌 참고용 차트입니다. 종목을 추가하려면 `TICKERS` 배열에 항목을 추가하면 됩니다.
...
js/data.js        # 10개 종목 저장 차트 데이터 + 시리즈 생성
js/track.js       # 차트 → 3D 레일/지지대/승강장/배경(잔디·나무·구름)
js/effects.js     # 폭죽·잔해·별 파티클
js/audio.js       # Web Audio 효과음 합성 (굉음/사이렌/비명/폭죽 등)
